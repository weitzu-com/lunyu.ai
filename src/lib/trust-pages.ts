    ...
    return [
      {
        ...
        body: [
          t(
            locale,
            `lunyu.ai 以《论语》句子为最小阅读单位，提供简体原文、${contentCoverage.reviewedGuide.complete ? "审校白话导读" : `白话导读（已审校 ${contentCoverage.reviewedGuide.ratio}）`}、James Legge 公版英译、注释、索引和听读入口。`,
            `lunyu.ai uses each Analects passage as the smallest reading unit, presenting simplified Chinese source text, ${contentCoverage.reviewedGuide.complete ? "a reviewed modern Chinese guide" : `a modern Chinese guide reviewed for ${contentCoverage.reviewedGuide.ratio} passages`}, James Legge's public-domain English translation, notes, indexes, and listening entry points.`
          ),
          t(
            locale,
    ...
    return [
      {
        ...
        body: [
          ...
          t(
            locale,
            `听读页已上线真人女声音频；当前 ${contentCoverage.audio.ratio} 章可播放，未录章节会被明确标注，不导向 404。逐句拼音${contentCoverage.pinyin.complete ? "已全量展示" : `当前覆盖 ${contentCoverage.pinyin.ratio}`}。`,
            `The listening page includes recorded audio; ${contentCoverage.audio.ratio} chapters are playable, unrecorded chapters are labeled instead of leading to 404s, and passage-level pinyin ${contentCoverage.pinyin.complete ? "is fully rendered" : `currently covers ${contentCoverage.pinyin.ratio} passages`}.`
          ),
        ],
      },
      {
...
export function faqItems(locale: Locale): FaqItem[] {
  return [
    ...
    },
    {
      ...
      answer: t(
        locale,
        `白话导读以 1948《白话论语读本》传统为参考，并通过站内编辑工作流整理审校；当前审校覆盖 ${contentCoverage.reviewedGuide.ratio}，不会把本地启发或 AI 输出伪装成原文解释。`,
        `The modern Chinese guide references the 1948 Baihua Lunyu Duben tradition and is organized and reviewed through the site workflow; current reviewed coverage is ${contentCoverage.reviewedGuide.ratio}, and local reflection or AI output is not presented as source interpretation.`
      ),
    },
    {
    ...
    },
    {
      ...
      answer: t(
        ...
        contentCoverage.pinyin.complete
          ? `是，逐句拼音已全量展示为 ${contentCoverage.pinyin.ratio}；页面会默认给出拼音，但原文、导读和译文仍保持分层。`
          : `尚未全量；逐句拼音当前覆盖 ${contentCoverage.pinyin.ratio}，页面只对已有数据的章节展示拼音。`,
        contentCoverage.pinyin.complete
          ? `Yes, passage-level pinyin is fully rendered at ${contentCoverage.pinyin.ratio}; pages show pinyin by default, while source text, guide text, and translations remain layered.`
          : `Not yet; passage-level pinyin currently covers ${contentCoverage.pinyin.ratio} passages and is shown only where source data is available.`
      ),
    },
    {
      ...
      answer: t(
        locale,
        `当前录音覆盖 ${contentCoverage.audio.ratio} 章；未录章节会在列表中禁用，避免误点到 404。`,
        `Recorded audio currently covers ${contentCoverage.audio.ratio} chapters; unavailable rows are disabled to avoid 404s.`
      ),
    },
    {
